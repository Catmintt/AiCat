<template>
  <section class="tool-category-section" id="video-tools" v-show="filteredTools.length > 0">
    <header class="section-header">
      <h2 class="section-title">🎬 AI视频工具</h2>
...
const filteredTools = computed(() => {
  if (!props.searchKeyword.trim()) return videoTools;
  const keyword = props.searchKeyword.toLowerCase();
  return videoTools.filter(tool => 
